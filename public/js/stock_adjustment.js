                    ...
                    if (ui.item.qty_available > 0) {
                        ...
                        swal({
                            ...
                            if (quantity !== null && quantity > 0) {
                                stock_adjustment_product_row(ui.item.variation_id, quantity);
                            } else {
...
function stock_adjustment_product_row(variation_id, quantity) {
    ...
    $.ajax({
        ...
        data: {
            ...
            quantity: quantity, // Pass the quantity to the server-side
        },
        dataType: 'html',
        success: function (result) {
            var $row = $(result);

            // Find the quantity input field and set its value
            $row.find('input.product_quantity').val(quantity);

            // Calculate the total (Quantity x Unit Price)
            var unit_price = parseFloat($row.find('input.product_unit_price').val());
            var row_total = quantity * unit_price;

            // Set the total in the appropriate field
            $row.find('input.product_line_total').val(row_total.toFixed(2));
        ...
        error: function () {
            swal('Error', 'Failed to add product row.', 'error');
        },
    });
}
