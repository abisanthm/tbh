...
$(document).ready(function () {
    ...
    $('button.pos-express-finalize').click(function () {
        ...
        // Reset the modal values and state
    ...
    $('#received_amount').on('input', function () {
        ...
        if (!isNaN(receivedAmount)) {
            let balance = receivedAmount - totalPayable;
            if (balance >= 0) {
                $('#balance_to_return')
                ...
                $('#balance_to_return')
                    ...
                    .text(
                        'Received amount is less than total payable. Please enter sufficient amount.'
                    );
            }
        } else {
            $('#balance_to_return').addClass('d-none');
        }
    });

    // Handle Print & Finalize button click
    $('#printButton').click(function () {
        let receivedAmount = parseFloat($('#received_amount').val());
        let totalPayable = parseFloat(__read_number($('input#final_total_input')));

        // Validate received amount before proceeding
        if (isNaN(receivedAmount) || receivedAmount < totalPayable) {
            toastr.error('Received amount is insufficient. Please enter the correct amount.');
            return false;
        }
