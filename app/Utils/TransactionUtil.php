...
class TransactionUtil extends Util
{
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->select(
                ...
                DB::raw('GROUP_CONCAT(DISTINCT tsl.sell_line_note SEPARATOR ", ") as sell_line_note') // Adding sell_line_note
            );
    ...
    public function getCakeListSells($business_id, $sale_type = 'sell')
{
    // Query for getting transactions with specific product_id = 284
    $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
        ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
        ->leftJoin('users as ss', 'transactions.res_waiter_id', '=', 'ss.id')
        ->leftJoin('users as dp', 'transactions.delivery_person', '=', 'dp.id')
        ...
        ->whereHas('sell_lines', function ($query) {
            $query->where('product_id', 284); // Filtering to only include transactions that have the product with ID 284
        })
        ->select(
            'transactions.id',
            'transactions.transaction_date',
        ...
        ->groupBy('transactions.id'); // Group by transaction ID to avoid duplicate entries
