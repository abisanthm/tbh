                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], ['id' => 0]), // Replace 1 with a dynamic value if needed
                                __('Product Stock Report'),
                                ['icon' => '', 'active' => request()->segment(2) == 'stock-history']
                            );
